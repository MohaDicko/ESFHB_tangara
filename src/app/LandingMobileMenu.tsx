...
export default function LandingMobileMenu() {
  ...
  return (
    <>
      ...
      {/* Menu Drawer */}
      <div 
        className={`fixed top-0 right-0 z-[70] h-full w-[85%] max-w-sm bg-white shadow-2xl transition-transform duration-500 ease-in-out md:hidden flex flex-col ${
      ...
      >
        ...
        <nav className="flex-1 px-6 py-8 flex flex-col gap-6">
          ...
          <Link 
            href="/login" 
            onClick={closeMenu}
            className="text-lg font-bold text-zinc-500 hover:text-black transition-colors"
          >
            Connexion
          </Link>
        </nav>
        ...
        <div className="p-6 border-t border-zinc-100">
          <Link 
            href="/register" 
            onClick={closeMenu}
            className="w-full bg-indigo-600 text-white p-5 rounded-2xl text-center font-bold shadow-xl shadow-indigo-200 flex items-center justify-center gap-3"
          >
