...
export default async function DirectoryPage({
  ...
  searchParams: { q?: string, promo?: string, status?: string }
}) {
  ...
  if (searchParams.promo) {
    query = query.eq('promo_year', searchParams.promo)
  }
       ...
       <div className="space-y-6">
          ...
          <form className="flex flex-col md:flex-row gap-4 bg-white p-4 rounded-[32px] border border-zinc-100 shadow-sm">
             ...
             <div className="flex gap-4">
                <select 
                  name="promo"
                  defaultValue={searchParams.promo}
              ...
              <div key={person.id} className="group p-8 bg-white border border-zinc-100 rounded-[40px] hover:border-brand/20 hover:shadow-2xl hover:shadow-brand/5 transition-all relative overflow-hidden">
                 <div className="flex flex-col items-center text-center space-y-6 relative z-10">
                    ...
                    <div className={`px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest ${
                      person.status === 'En poste' ? 'bg-green-50 text-green-600' : 'bg-brand/5 text-brand'
                    }`}>
                       {person.status || 'Non renseigné'}
                    </div>
                 ...
                 <div className="absolute -bottom-4 -right-4 opacity-0 group-hover:opacity-10 transition-opacity">
                    <GraduationCap size={120} className="text-brand" />
                 </div>
              </div>
            ))}
         </div>
